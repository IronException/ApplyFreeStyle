...
public class TextManager extends Offset{
  ...
  /*
  public TextManager setWidth(float xSize){
    super.setXSize(xSize);
    //setWVision(true);
    return this;
  }
  
  public TextManager setHeight(float ySize){
  ...
  public void init(){
    if(!init){
      ...
      //println("last");
      txtSize = getTextSize(txt, xOffSize, yOffSize);
      //println("but");
    } else {
      textSize(txtSize);
    }
  ...
  public float getTextSize(String text, float xSize, float ySize){
    ...
    // init above
    // for what?
    while(doesTextSizeFit(above, text, xSize, ySize)){
      above += ySize;
      //println(above);
    }
    //println("where");
